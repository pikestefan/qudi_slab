...
class Pulse(GenericLogic):
    ...
    def setup_and_play(self, laser_times, mw_times, apd_times, rep=100, rest=100, clk_rate=100, mw_pulse=bool, trigger=bool):
        '''
        ...
        # convert everything in s immediately
        laser_in=2,wait=1,laser_re=7,
        apd_stop=2, start=3, steps=1,
         rest=100,
         mw_wait_time=2.5, mw_pulse_time=0.25,
         rep=10,
         mw_pulse=True, trigger=True)
        '''
        print('mw array before: ', mw_times)
        laser_times = np.multiply(laser_times, 1e-3)
        mw_times = np.multiply(mw_times, 1e-3)
        print('mw array: ', mw_times)
        ...
        #if not given differently put the clk_rate to 100
        if clk_rate == 100:
            clk_rate= MEGA(100)
        ...
        segment_map=[]
        # calculate the whole length of the waveform --> whole_wf
        # The parameters for the laser determine how long the waveform is
        msplay = laser_times[0] + laser_times[1] + laser_times[2] + rest  # in s
        # print('msplay: ', msplay)

        #use all these parameters and put it to the general sweep function:
        self.delay_sweep_general(card_idx, clk_rate, msplay, laser_times[0], laser_times[1], laser_times[2], rest, apd_times[2], apd_times[0], apd_times[1], mw_times[0],
                           mw_times[1], rep, channels, mw_pulse, trigger, segment_map)


    # This is the main general block...works fine with the laser
    def delay_sweep_general(self, card_idx, clk_rate, msplay, laser_in, wait, laser_re, rest, apd_stop, start, steps, mw_wait_time,
                           mw_pulse_time, rep, channels, mw_pulse=bool, trigger=bool, segment_map=[]):
            ...
            # print('msplay: ', msplay)
            # print('rest: ', rest)
            # print('apd_stop: ', apd_stop)
            stop = (msplay - rest) - apd_stop
            # print('mw pulse time ', mw_pulse_time)
            ...
            for i in apd_start:
                ...
                if mw_pulse == True:
                    ...
                    # print('pi_puls_array: ', pi_pulse_array)
                    mw_off = np.zeros(self._pulser.waveform_padding(clk_rate * msplay - (mw_wait_time + mw_pulse_time)))
                    pi_pulse_array = np.subtract(pi_pulse_array, 0.5)  # because the MW takes only +-0.5V
                    mw_ao_waveform = np.concatenate((mw_wait, pi_pulse_array, mw_off))

                    print('ao value: ', mw_ao_waveform)
                    print('max of mw_ao_waveform: ', np.max(mw_ao_waveform))
            ...
            # load and play
            # print('len of dosequence: ', len(dosequence))h
            # print('len of aosequence: ', len(aosequence))
            digital_output_map = {
                0: [laser_outchan, apd_outchan]}  # directs to the right output channel analog channel0 directs to digital x0 and x1
    ...
    def mw_test(self, msecondsplay, loops, segment_map=[]):
        ...
        for aos, dos in zip(aosequence, dosequence):
            self.load_waveform(ao_waveform_dictionary=aos, do_waveform_dictionary=dos, digital_output_map=digital_output_map)
        ...
        loops = np.ones((len(aosequence)), dtype=np.int64)
        # print('loops: ', loops)
        stop_condition_list = np.array([SPCSEQ_ENDLOOPONTRIG, SPCSEQ_ENDLOOPONTRIG, SPCSEQ_ENDLOOPONTRIG, SPCSEQ_ENDLOOPONTRIG], dtype=np.int64)
