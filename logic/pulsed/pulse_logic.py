...
class Pulse(GenericLogic):
    ...
    def play_any(
        ...
        if method == "delaysweep":
            ...
            self.play_ramsey(
                seq_len, laser_times, apd_times, apd_ref, mw_times, rep, trigger
            )
            ...
            # print("It does Rabi.")
            self.play_rabi(
                seq_len, laser_times, apd_times, apd_ref, mw_times, rep, trigger, neg_mw
            )
        ...
        for i in mw_len:
            ...
            if seq_len < (mw_times[0] + mw_times[2]):
                ...
                if neg_mw:
                    ...
                    mw_on = np.ones(int(round(clk_rate * i)))

                rest_array = np.zeros(
                    int(round(clk_rate * (seq_len_s - (mw_times_s[0] + i))))
                )
    ...
    def play_ramsey(
        self, seq_len, laser_times, apd_times, apd_ref, mw_times, trigger=True, rep=100
    ):
        """
        ...
        for i in break_len:
            ...
            if seq_len < (laser_times[0] + laser_times[1] + laser_times[2]):
                ...
                # build mw waveform for every possible length
                mw_wait = np.zeros(int(round(clk_rate * mw_times_s[0])))
                mw_pulse = np.ones(int(round(clk_rate * mw_times_s[4])))
                # this is fix, use this 2 times
                mw_break = np.zeros(
                    int(round(clk_rate * i))
                )  # this changes duration min + i

                mw_all = mw_times_s[0] + mw_times_s[4] + i + mw_times_s[4]
                rest_array = np.zeros(int(round(clk_rate * (seq_len_s - mw_all))))
                mw_ao_waveform = np.concatenate(
                    (mw_wait, mw_pulse, mw_break, mw_pulse, rest_array)
                )
                len_mw_ao_waveform = self._pulser.waveform_padding(len(mw_ao_waveform))
                difference = len_mw_ao_waveform - len(mw_ao_waveform)
