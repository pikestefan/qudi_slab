...
class SpectrumNetbox(Base, PulserInterface):
    ...
    def sequence_test(self, msecondsplay, loops, segment_map=[0, 1, 2]):
        ...
        sigma = (time_ax[-1]) / 20
        second_seq_ch1 = np.exp(
            -((time_ax - time_ax[-1] / 2) ** 2) / (2 * sigma**2)
        ) * np.sin(20 * 2 * np.pi * time_ax / msecondsplay)
        ...
        stop_condition_list = np.array(
            [SPCSEQ_ENDLOOPONTRIG, SPCSEQ_ENDLOOPONTRIG, SPCSEQ_ENDLOOPONTRIG],
            dtype=np.int64,
        )
