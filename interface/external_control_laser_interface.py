...
class ExtCtrlLaserInterface(metaclass=InterfaceMetaclass):
    """ This interface can be used to control a simple laser. It handles power control, control modes and shutter states

    This interface expands on the simple laser interface class, to include the option of tuning the power with external
    sources (i.e. analog voltages and/or TTL signals). Power readout is read as an analog voltage coming from a photodiode.

    TODO: add the functions to enable laser TTL on/off control
    """
    ...
    def set_power_extctrl(self, voltage):
        """
        Send a voltage to the external laser controller to tune the power
        """
        pass
