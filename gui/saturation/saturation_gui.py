...
from pyqtgraph import SignalProxy, mkColor, mkPen
...
from gui.colordefs import QudiPalettePale, QudiPalette
...
class SaturationGui(GUIBase):
    ...
    saturation_logic = Connector(interface="SaturationLogic")
    ...
    def on_activate(self):
        ...
        self._saturation_logic = self.saturation_logic()

        if (
            not isinstance(self._pen_color_list, (list, tuple))
            or len(self._pen_color_list) < 1
        ):
            self.log.warning(
        ...
        self.sigStartSaturationMeasurement.connect(
            self._saturation_logic.measure_saturation, QtCore.Qt.QueuedConnection
        )
        self.sigRemovePlotClicked.connect(
            self._plot_logic.remove_plot, QtCore.Qt.QueuedConnection
        )
        ...
        self._plot_logic.sigFitUpdated.connect(
            self.update_fit_data, QtCore.Qt.QueuedConnection
        )
        self._saturation_logic.sigMeasuredSaturation.connect(
            self.receive_saturation_data, QtCore.Qt.QueuedConnection
        )
    ...
    def update_data(self, plot_index, x_data=None, y_data=None, clear_old=None):
        ...
        if not (0 <= plot_index < len(self._plot_dockwidgets)):
            self.log.warning(
                "Tried to update plot with invalid index {0:d}".format(plot_index)
            )
            return
        ...
        for line, xd in enumerate(x_data):
            ...
            self._plot_curves[plot_index].append(
                dockwidget.plot_PlotWidget.plot(
                    pen=None, symbol="o", symbolSize=8, symbolBrush=QudiPalette.blue,
                )
            )
            self._plot_curves[plot_index][-1].setData(x=xd, y=yd)
            self._fit_curves[plot_index].append(dockwidget.plot_PlotWidget.plot(
                pen=mkPen(QudiPalettePale.orange, style=QtCore.Qt.DotLine),
            ))
    ...
    def update_fit_data(
        self, plot_index, fit_data=None, formatted_fitresult=None, fit_method=None
    ):
    ...
    def _pyqtgraph_x_limits_changed(self, plot_index, limits):
        plot_item = (
            self._plot_dockwidgets[plot_index].widget().plot_PlotWidget.getPlotItem()
        )
